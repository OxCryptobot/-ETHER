        ...
        try:
            ...
            if getattr(payload, "action", "") == "health" and self._client is None:
                ...
                offline = CitrineResponse(
                    results=[RetrievalResult(id="health", text="offline", score=0.0, metadata=h)],
            ...
            if self.client is None:
                return ResponseEnvelope(
                    ...
                    error=GemError(
                        ...
                        recoverable=True,
                        suggested_action="Start Qdrant or set QDRANT_URL",
                    ),
                )
    ...
    def _add(self, collection: str, documents: List[Dict[str, Any]]) -> int:
        ...
        for doc in documents:
            ...
            for piece in pieces:
                ...
                points.append(qmodels.PointStruct(id=str(uuid4()), vector=vec, payload=payload))
        if not points:
            return 0

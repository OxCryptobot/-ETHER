...
def _commit_done(items_with_results: list) -> int:
    def _inner(data: dict):
        for item, result in items_with_results:
            data.setdefault("done", []).append({**item, "result": result})
        data["done"] = data["done"][-200:]
        return data, len(items_with_results)

    return mutate(_inner)


def drain(limit: int = 1) -> dict:
    claimed = _claim(max(1, limit))
    if not claimed:
